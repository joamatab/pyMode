...
def parseInput():
    ...
    parser.add_argument(
        "--dcore", type=float, default=5.1e-3
    )  # resolution in x dimension in microns
    parser.add_argument(
        "--dcladding", type=float, default=30.1e-3
    )  # resolution in y dimension in microns
    parser.add_argument(
        "--xWidth", type=float, default=5
    )  # domain size in x dimension in microns
    parser.add_argument(
        "--yWidth", type=float, default=3
    )  # domain size in y dimension in microns
    parser.add_argument(
        "--numModes", type=int, default=1
    )  # number of modes to solve for
    parser.add_argument(
        "--folderName", type=str, default=""
    )  # where to output the results to. Note: Must have / at the end.
    parser.add_argument(
        "--filenamePrefix", type=str, default=""
    )  # whether or not this is a testing run for debugging
...
if __name__ == "__main__":
    ...
    for params in s.params(start, stop):
        ...
        # run the iteration
        neff[k, 0] = runSimulation(
            **params, dcore=dcore, dcladding=dcladding, xWidth=xWidth, yWidth=yWidth
        )
        k += 1
    print(neff)
    # Save the data
    np.savez(
        folderName + filenamePrefix + "_data.npz",
        wavelength=wavelength,
        width=width,
        thickness=thickness,
        sidewallAngle=sidewallAngle,
        neff=neff,
    )
