...
for k in range(len(dcore)):
    ...
    bot = width + 2 * (
        thickness / np.tan(sidewallAngle_radians)
    )  # size of the bottom of waveguide
    # print(bot)
    ...
    xLocs = [
        ...
        bot / 2 + 0.1,
        xWidth / 2,
    ]  # where the densities should change
    xVals = [
        dcore[k],
        dcore[k],
        dcladding,
        dcladding,
    ]  # densities at those locations (should be 1 more than above)
    xLeft = make_grid(xLocs, xVals)  # makes the actual grid from left side
    xx = xLeft - xLeft[-1]
    xx = np.concatenate(
        (-(np.flip(xLeft[1:-1], 0)), xLeft)
    )  # use if not using boundary condition
    xx = xLeft

    # same thing as before
    yLocs = [0, thickness / 2, thickness / 2 + 0.1, yWidth / 2]
    yVals = [dcore[k], dcore[k], dcladding, dcladding]
    yLeft = make_grid(yLocs, yVals)
    yy = yLeft - yLeft[-1]
